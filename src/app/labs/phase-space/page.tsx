      ...
      <div className="hidden md:flex flex-col h-full">
        ...
        <header className="h-14 border-b border-neutral-900 flex items-center justify-between px-6 bg-neutral-950 shrink-0">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-sm font-bold tracking-tight text-white">
              ...
              <span className="text-neutral-400 font-medium">Phase Space Biophysics</span>
            </h1>
          </div>
          <div className="flex items-center gap-4">
        ...
        <main className="flex-1 flex overflow-hidden p-6 gap-6">
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.15em] text-neutral-600 font-mono">FitzHugh-Nagumo Model</span>
                  </div>
                  <div className="bg-black/30 rounded-xl p-4 flex flex-col items-center justify-center border border-neutral-800/30 text-neutral-200 gap-2">
                    <div
                      className={cn("p-2 rounded transition-colors w-full flex justify-center", hoveredTerm === 'dv' ? 'bg-teal-950/30 text-teal-400' : '')}
                      onMouseEnter={() => setHoveredTerm('dv')} onMouseLeave={() => setHoveredTerm(null)}
                    >
                      <BlockMath math="\frac{dv}{dt} = v - \frac{v^3}{3} - w + I_{ext}" />
                    </div>
                    <div
                      className={cn("p-2 rounded transition-colors w-full flex justify-center", hoveredTerm === 'dw' ? 'bg-teal-950/30 text-teal-400' : '')}
              ...
              <div className="space-y-4 pt-6 mt-6 border-t border-neutral-800/50">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center text-xs font-mono bg-neutral-950/50 p-2 rounded border border-teal-500/20 gap-3">
                    ...
                    />
                    <span className="text-teal-500 font-bold w-12 text-right tabular-nums">{params.I.toFixed(2)}</span>
                  </div>
                  ...
                  <div className="flex justify-between items-center text-xs font-mono bg-neutral-950/50 p-2 rounded border border-neutral-800/50 gap-3">
                    <span className="text-neutral-400 w-24 shrink-0">Param (a)</span>
                    <Slider
                    ...
                    />
                    <span className="text-neutral-300 font-bold w-12 text-right tabular-nums">{params.a.toFixed(2)}</span>
                  </div>

                  <div className="flex justify-between items-center text-xs font-mono bg-neutral-950/50 p-2 rounded border border-neutral-800/50 gap-3">
                    <span className="text-neutral-400 w-24 shrink-0">Param (b)</span>
                    <Slider
                      min={0} max={1} step={0.01}
                      value={[params.b]}
                      ...
                      className="flex-1 cursor-pointer"
                    />
                    <span className="text-neutral-300 font-bold w-12 text-right tabular-nums">{params.b.toFixed(2)}</span>
                  </div>

                  <div className="flex justify-between items-center text-xs font-mono bg-neutral-950/50 p-2 rounded border border-neutral-800/50 gap-3">
                    <span className="text-neutral-400 w-24 shrink-0">Time (τ)</span>
                    <Slider
                      ...
                      className="flex-1 cursor-pointer"
                    />
                    <span className="text-neutral-300 font-bold w-12 text-right tabular-nums">{params.tau.toFixed(1)}</span>
                  </div>
                </div>
              </div>
          ...
          <section className="flex-1 flex flex-col gap-6 min-w-0">
            {/* Phase Portrait */}
            <div className="flex-[2] bg-neutral-900/30 border border-neutral-800 rounded-2xl overflow-hidden flex flex-col relative shadow-inner p-4">
              <div className="absolute top-4 left-4 z-10 pointer-events-none select-none flex flex-col">
                <span className="text-sm font-black text-neutral-500 font-mono tracking-tighter uppercase">
                  Phase Portrait (V vs W)
                </span>
                <div className="flex items-center gap-4 mt-2 text-[10px] font-mono font-bold">
                  <div className="flex items-center gap-1"><div className="w-2 h-2 bg-teal-500 rounded-full"></div><span className="text-teal-500">V-Nullcline</span></div>
                  <div className="flex items-center gap-1"><div className="w-2 h-2 bg-teal-500 rounded-full"></div><span className="text-teal-500">W-Nullcline</span></div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                  ...
                  <Scatter data={vNullcline} fill="#14b8a6" line={{ stroke: '#14b8a6', strokeWidth: 2 }} shape={() => <></>} />
                  <Scatter data={wNullcline} fill="#5eead4" line={{ stroke: '#5eead4', strokeWidth: 2 }} shape={() => <></>} />

                  {/* Trajectory */}
                  <Scatter data={phaseData} fill="#ffffff" line={{ stroke: '#ffffff', strokeWidth: 2 }} shape={() => <></>} />

                  {/* Current State Point */}
                  <Scatter data={[{ v, w }]} fill="#737373" shape="circle" />
                </ScatterChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="flex-[1] bg-neutral-900/30 border border-neutral-800 rounded-2xl overflow-hidden flex flex-col relative shadow-inner p-4">
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={history} margin={{ top: 10, right: 10, bottom: 10, left: 10 }}>
                  ...
                  <Line type="monotone" dataKey="w" stroke="#5eead4" strokeWidth={2} dot={false} isAnimationActive={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </section>
