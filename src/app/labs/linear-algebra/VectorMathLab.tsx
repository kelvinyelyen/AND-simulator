                ...
                <div className="bg-neutral-900/50 border border-neutral-800 p-6 rounded-2xl space-y-8 flex flex-col shadow-sm h-full overflow-y-auto [&::-webkit-scrollbar]:hidden">
                    ...
                    <div className="space-y-6 pt-4 border-t border-neutral-800/50">
                        <div className="flex items-center justify-between">
                            ...
                            <span className="text-[10px] uppercase tracking-wider text-teal-400 font-mono animate-pulse">
                                Live Signal
                            </span>
                        </div>
                        ...
                        <div className="space-y-2">
                            {inputs.map((val, i) => (
                                <div key={i} className="flex justify-between items-center text-xs font-mono bg-neutral-950/50 p-2 rounded border border-neutral-800/50 gap-3">
                                    ...
                                    <span className="text-xs font-bold text-teal-400 w-12 text-right tabular-nums">
                                        {val.toFixed(2)}
                                    </span>
                                </div>
                            ))}
                                ...
                                <div key={i} className="flex justify-between items-center text-xs font-mono bg-neutral-950/50 p-2 rounded border border-neutral-800/50 gap-3">
                                    ...
                                    <Slider
                                        ...
                                        className={cn(
                                            ...
                                            w < 0 ? "[&_[role=slider]]:bg-red-500" : "[&_[role=slider]]:bg-teal-500"
                                        )}
                                    />

                                    <span className={cn(
                                        "text-xs font-bold w-12 text-right tabular-nums",
                                        w < 0 ? "text-red-400" : "text-teal-400"
                                    )}>
                                        {w.toFixed(2)}
                                    </span>
                                </div>
                    ...
                    {/* Result Description */}
                    <div className="p-3 rounded-lg bg-neutral-950/50 border border-neutral-800/50">
                        <p className="text-[11px] font-medium leading-tight text-teal-400/90">
                            {`Magnitude: `}
                            <span className="tabular-nums inline-block w-8 text-center font-bold">
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-6 gap-12 overflow-hidden">
                    <div className="flex items-end justify-center w-full gap-8 xl:gap-16">
                        ...
                        <div className="flex gap-8 items-end">
                            {inputs.map((inVal, i) => (
                                <div key={i} className="flex flex-col items-center gap-4">
                                    <div className="w-14 bg-neutral-900 border border-neutral-800 rounded-lg overflow-hidden relative h-48 md:h-64 shadow-xl group cursor-pointer">
                                        {[0.25, 0.5, 0.75].map((tick) => (
                                            <div key={tick} className="absolute w-full h-px bg-neutral-800/50" style={{ bottom: `${tick * 100}%` }} />
                                        ))}
                                        <div
                                            className="absolute bottom-0 w-full bg-teal-500/60 transition-all duration-100 ease-linear border-t border-teal-400 group-hover:bg-teal-400/60"
                        ...
                        <div className="h-48 md:h-64 flex flex-col justify-center">
                            <span className="text-neutral-800 text-4xl font-light">→</span>
                        </div>

                        {/* Output Vector */}
                        <div className="flex flex-col items-center gap-4">
                            <div className="w-24 h-48 md:h-64 bg-neutral-900 border-2 border-neutral-800 rounded-xl relative overflow-hidden shadow-2xl">
                                {[0.25, 0.5, 0.75].map((tick) => (
                                    <div key={tick} className="absolute w-full h-px bg-neutral-800" style={{ bottom: `${tick * 100}%` }} />
                                ))}
                                <div
                                    className={cn(
                                        "absolute bottom-0 w-full transition-all duration-300 ease-out border-t",
                                        dotProduct < 0 ? "bg-teal-500/50 border-teal-400" : "bg-teal-500/50 border-teal-400"
                                    )}
                                    style={{ height: `${Math.min(Math.abs(dotProduct) * 33, 100)}%` }}
                                />
                            </div>
                            <span className="text-[10px] font-mono text-neutral-500 font-bold uppercase tracking-widest text-center">
                                {useMemo(() => <InlineMath math="y" />, [])}
                            </span>
                        </div>
                    </div>

                    {/* Readout Formula Bar */}
                    <div className="w-full max-w-4xl bg-neutral-950/50 border border-neutral-800 rounded-xl p-6 shadow-sm backdrop-blur-md">
                        <div className="flex flex-wrap items-center justify-center gap-x-3 font-mono text-sm xl:text-lg">
                            <span className="text-neutral-600 italic">{useMemo(() => <InlineMath math="y =" />, [])}</span>
                            {weights.map((w, i) => (
                                <React.Fragment key={i}>
                                    {i > 0 && <span className="text-neutral-700">+</span>}
                                    <div className="flex items-center gap-2 bg-neutral-900/80 px-3 py-1.5 rounded-lg border border-neutral-800/50">
                                        <span className={cn("font-bold w-12 text-right tabular-nums inline-block", w < 0 ? "text-red-400" : "text-teal-400")}>{w.toFixed(2)}</span>
                                        <span className="text-neutral-700">·</span>
                                        <span className="text-teal-400 w-12 text-right tabular-nums inline-block">{inputs[i].toFixed(2)}</span>
                                    </div>
                                </React.Fragment>
                            ))}
                            <span className="text-neutral-700">=</span>
                            <span className={cn("font-bold text-xl w-20 text-center tabular-nums inline-block", dotProduct < 0 ? "text-teal-400" : "text-teal-400")}>{dotProduct.toFixed(2)}</span>
                        </div>
                    </div>
                </div>
            </section>
