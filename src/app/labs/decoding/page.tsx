        ...
        <header className="h-14 border-b border-neutral-900 flex items-center justify-between px-6 bg-neutral-950 shrink-0">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-sm font-bold tracking-tight text-white">
              ...
              <span className="text-neutral-400 font-medium">Neural Decoding</span>
            </h1>
          </div>
          <div className="flex items-center gap-4">
        ...
        <main className="flex-1 flex overflow-hidden p-6 gap-6">
            ...
            <div className="h-full flex flex-col p-6 overflow-y-auto [&::-webkit-scrollbar]:hidden">
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.15em] text-neutral-600 font-mono">Poisson Spiking</span>
                  </div>
              ...
              <div className="space-y-4 pt-6 mt-6 border-t border-neutral-800/50">
                ...
                <div className="space-y-3">
                  <div className="flex flex-col bg-neutral-950/50 p-4 rounded-xl border border-teal-500/30 gap-4">
                    <div className="flex justify-between items-center text-xs font-mono">
                      <span className="text-teal-400 font-bold tracking-widest uppercase">True Angle <InlineMath math="\theta" /></span>
                      <span className="text-white font-black text-lg">{params.trueAngle}°</span>
                    </div>
                    <Slider min={0} max={359} step={1} value={[params.trueAngle]} onValueChange={(val) => setParams({ trueAngle: val[0] })} className="[&_[role=slider]]:bg-teal-400" />
                  </div>
                  <Button onClick={triggerStimulus} className="w-full bg-neutral-800 hover:bg-neutral-700 text-neutral-300 font-bold h-10 border border-neutral-700">
              ...
              <div className="space-y-4 pt-6 mt-6 border-t border-neutral-800/50">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center text-xs font-mono bg-neutral-950/50 p-2 rounded border border-neutral-800/50 gap-3">
                    <span className="text-neutral-400 w-24 shrink-0">Spread <InlineMath math="\sigma" /></span>
                    <Slider min={5} max={90} step={5} value={[params.sigma]} onValueChange={(val) => setParams({ sigma: val[0] })} className="flex-1" />
                    <span className="text-neutral-300 font-bold w-12 text-right tabular-nums">{params.sigma}°</span>
                  </div>
                  <div className="flex justify-between items-center text-xs font-mono bg-neutral-950/50 p-2 rounded border border-neutral-800/50 gap-3">
                    <span className="text-neutral-400 w-24 shrink-0">Time Win</span>
                    <Slider min={50} max={1000} step={50} value={[params.timeWindow]} onValueChange={(val) => setParams({ timeWindow: val[0] })} className="flex-1" />
                    <span className="text-neutral-300 font-bold w-12 text-right tabular-nums">{params.timeWindow}ms</span>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-8 space-y-3">
                <div className="flex justify-between items-center bg-teal-950/30 p-3 rounded-lg border border-teal-900/50">
                  <span className="text-xs text-teal-500 font-bold uppercase tracking-widest">Decoded <InlineMath math="\hat{\theta}" /></span>
                  <span className="text-teal-400 font-black text-xl">{decodedAngle.toFixed(1)}°</span>
                </div>
                <div className="flex justify-between items-center bg-teal-950/30 p-3 rounded-lg border border-teal-900/50">
                  <span className="text-xs text-teal-500 font-bold uppercase tracking-widest">Error</span>
                  <span className="text-teal-400 font-black text-lg">{error.toFixed(1)}°</span>
                </div>
              </div>
          ...
          <section className="flex-1 flex flex-col gap-6 min-w-0">
            ...
            <div className="flex-[1] bg-neutral-900/30 border border-neutral-800 rounded-2xl overflow-hidden flex flex-col relative shadow-inner p-4">
              <span className="absolute top-4 left-4 z-10 text-sm font-black text-neutral-500 font-mono uppercase tracking-tighter">Population Response (Spike Count vs Preferred Angle)</span>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={barData} margin={{ top: 40, right: 20, bottom: 20, left: 10 }}>
                  ...
                  <ReferenceLine x={params.trueAngle} stroke="#d4d4d8" strokeWidth={2} strokeDasharray="3 3" label={{ position: 'top', value: 'TRUE', fill: '#d4d4d8', fontSize: 10 }} />
                  <ReferenceLine x={decodedAngle} stroke="#14b8a6" strokeWidth={2} label={{ position: 'top', value: 'DECODED', fill: '#14b8a6', fontSize: 10 }} />
                  <Bar dataKey="spikes" radius={[4, 4, 0, 0]}>
                    {barData.map((entry, index) => (
                      <Cell key={`cell-${index}`} fill="#5eead4" opacity={0.8} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
