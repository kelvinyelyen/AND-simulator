        ...
        <header className="h-14 border-b border-neutral-900 flex items-center justify-between px-6 bg-neutral-950 shrink-0">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-sm font-bold tracking-tight text-white">
              ...
              <span className="text-neutral-400 font-medium">STDP & Hebbian Learning</span>
            </h1>
          </div>
          <div className="flex items-center gap-4">
        ...
        <main className="flex-1 flex overflow-hidden p-6 gap-6">
            ...
            <div className="h-full flex flex-col p-6 overflow-y-auto [&::-webkit-scrollbar]:hidden">
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.15em] text-neutral-600 font-mono">Learning Rule</span>
                  </div>
                  <div className="bg-black/30 rounded-xl p-4 flex flex-col items-center justify-center border border-neutral-800/30 text-neutral-200">
                    ...
                    <div className="text-xs text-neutral-500 mt-2"><InlineMath math="\Delta t = t_{post} - t_{pre}" /></div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4 pt-6 mt-6 border-t border-neutral-800/50">
                ...
                <div className="flex gap-4">
                  <Button onClick={() => triggerSpike('pre')} className="flex-1 bg-teal-600 hover:bg-teal-500 text-white font-bold h-12">
                    PRE Spike (J)
                  </Button>
                  <Button onClick={() => triggerSpike('post')} className="flex-1 bg-teal-600 hover:bg-teal-500 text-white font-bold h-12">
                    POST Spike (K)
                  </Button>
                </div>
              </div>
              ...
              <div className="space-y-4 pt-6 mt-6 border-t border-neutral-800/50">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center text-xs font-mono bg-neutral-950/50 p-2 rounded border border-neutral-800/50 gap-3">
                    <span className="text-neutral-400 w-24 shrink-0">LTP <InlineMath math="A_+" /></span>
                    <Slider min={0} max={0.5} step={0.01} value={[params.A_plus]} onValueChange={(val) => setParams({ A_plus: val[0] })} className="flex-1" />
                    <span className="text-neutral-300 font-bold w-12 text-right tabular-nums">{params.A_plus.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between items-center text-xs font-mono bg-neutral-950/50 p-2 rounded border border-neutral-800/50 gap-3">
                    <span className="text-neutral-400 w-24 shrink-0">LTD <InlineMath math="A_-" /></span>
                    <Slider min={0} max={0.5} step={0.01} value={[params.A_minus]} onValueChange={(val) => setParams({ A_minus: val[0] })} className="flex-1" />
                    <span className="text-neutral-300 font-bold w-12 text-right tabular-nums">{params.A_minus.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between items-center text-xs font-mono bg-neutral-950/50 p-2 rounded border border-neutral-800/50 gap-3">
                    <span className="text-neutral-400 w-24 shrink-0">Time <InlineMath math="\tau" /></span>
                    ...
                    <span className="text-neutral-300 font-bold w-12 text-right tabular-nums">{params.tau_plus}</span>
                  </div>
                </div>
              </div>

              {/* Visual STDP Curve inside sidebar */}
              <div className="mt-8 h-40 bg-black/20 rounded-xl p-2 border border-neutral-800/30">
                <span className="text-[10px] text-neutral-500 uppercase font-bold ml-2">STDP Window</span>
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={stdpCurveData}>
                    ...
                    <Line type="monotone" dataKey="dw" stroke="#14b8a6" strokeWidth={2} dot={false} isAnimationActive={false} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
          ...
          <section className="flex-1 flex flex-col gap-6 min-w-0">
            ...
            <div className="flex-[1] bg-neutral-900/30 border border-neutral-800 rounded-2xl overflow-hidden flex flex-col relative shadow-inner p-4">
              <span className="absolute top-4 left-4 z-10 text-sm font-black text-neutral-500 font-mono uppercase tracking-tighter">Event Timeline</span>
              <div className="absolute top-4 right-4 z-10 flex gap-4 text-[10px] font-bold font-mono">
                <span className="text-teal-500 flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-teal-500"></div> Pre-Spikes</span>
                <span className="text-teal-500 flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-teal-500"></div> Post-Spikes</span>
              </div>
              <ResponsiveContainer width="100%" height="100%">
                <ScatterChart margin={{ top: 40, right: 20, bottom: 20, left: 20 }}>
                  ...
                  <ZAxis range={[100, 100]} />
                  <Scatter data={preSpikesData} fill="#5eead4" shape="circle" isAnimationActive={false} />
                  <Scatter data={postSpikesData} fill="#14b8a6" shape="circle" isAnimationActive={false} />
                </ScatterChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="flex-[1.5] bg-neutral-900/30 border border-neutral-800 rounded-2xl overflow-hidden flex flex-col relative shadow-inner p-4">
              <span className="absolute top-4 left-4 z-10 text-sm font-black text-neutral-500 font-mono uppercase tracking-tighter">Synaptic Weight (w)</span>
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={weightHistory} margin={{ top: 30, right: 20, bottom: 20, left: 20 }}>
                  ...
                  <Line type="stepAfter" dataKey="weight" stroke="#a3a3a3" strokeWidth={3} dot={false} isAnimationActive={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>
