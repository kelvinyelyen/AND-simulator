        ...
        <header className="h-14 border-b border-zinc-900 flex items-center justify-between px-6 bg-zinc-950 shrink-0">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-base font-semibold tracking-tight text-white">
              ...
              <span className="text-zinc-400 font-medium">Synaptic Mechanics</span>
            </h1>
          </div>
          <div className="flex items-center gap-4">
        ...
        <main className="flex-1 flex overflow-hidden p-6 gap-6">
            ...
            <div className="h-full flex flex-col p-6 overflow-y-auto [&::-webkit-scrollbar]:hidden">
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.15em] text-zinc-600 font-mono">Synapse Equation</span>
                  </div>
              ...
              <div className="space-y-4 pt-6 mt-6 border-t border-zinc-800/50">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <Button onClick={triggerPreSpike} size="sm" className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold font-mono h-7 text-xs">
                    Fire Pre-Synapse
                  </Button>
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center text-xs font-mono bg-zinc-950/50 p-2 rounded border border-zinc-800/50 gap-3">
                    <span className="text-zinc-400 w-24 shrink-0">Decay <InlineMath math="\tau_{syn}" /></span>
                    <Slider
                      min={1} max={50} step={1}
                    ...
                    />
                    <span className="text-zinc-300 font-bold w-12 text-right tabular-nums">{params.tau_syn} ms</span>
                  </div>

                  <div className="flex justify-between items-center text-xs font-mono bg-zinc-950/50 p-2 rounded border border-zinc-800/50 gap-3">
                    <span className="text-zinc-400 w-24 shrink-0">Reversal <InlineMath math="E_{syn}" /></span>
                    <Slider
                      ...
                      className="flex-1 cursor-pointer"
                    />
                    <span className="text-blue-400 font-bold w-12 text-right tabular-nums">{params.Esyn} mV</span>
                  </div>

                  <div className="flex justify-between items-center text-xs font-mono bg-zinc-950/50 p-2 rounded border border-zinc-800/50 gap-3">
                    <span className="text-zinc-400 w-24 shrink-0">Weight <InlineMath math="g_{max}" /></span>
                    <Slider
                      ...
                      className="flex-1 cursor-pointer"
                    />
                    <span className="text-amber-400 font-bold w-12 text-right tabular-nums">{params.g_max}</span>
                  </div>
                </div>
              </div>
          ...
          <section className="flex-1 flex flex-col gap-6 min-w-0">
            ...
            <div className="flex-[1] bg-zinc-900/30 border border-zinc-800 rounded-2xl overflow-hidden flex flex-col relative shadow-inner p-4">
              <span className="absolute top-4 left-4 z-10 text-sm font-black text-zinc-500 font-mono uppercase tracking-tighter">Synaptic Conductance (g_syn)</span>
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={history} margin={{ top: 20, right: 10, bottom: 10, left: 10 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#1f1f22" vertical={false} />
                  <XAxis dataKey="time" type="number" domain={['dataMin', 'dataMax']} hide />
                  <YAxis domain={[0, 'auto']} hide />
                  <Line type="monotone" dataKey="g_syn" stroke="#f59e0b" strokeWidth={2} dot={false} isAnimationActive={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>

            <div className="flex-[1] bg-zinc-900/30 border border-zinc-800 rounded-2xl overflow-hidden flex flex-col relative shadow-inner p-4">
              <span className="absolute top-4 left-4 z-10 text-sm font-black text-zinc-500 font-mono uppercase tracking-tighter">Post-Synaptic Voltage (mV)</span>
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={history} margin={{ top: 20, right: 10, bottom: 10, left: 10 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#1f1f22" vertical={false} />
                  <XAxis dataKey="time" type="number" domain={['dataMin', 'dataMax']} hide />
                  <YAxis domain={[-90, -30]} hide />
                  <ReferenceLine y={params.thresh} stroke="#ef4444" strokeDasharray="4 4" opacity={0.5} />
